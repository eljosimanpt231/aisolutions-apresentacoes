...
(function (global) {
  ...
  /* Escreve um valor e pisca o numero quando ele muda. */
  ...
  global.motorOrcamentacao = function () {
    ...
    function pintarEntradas(s) {
      ...
      set(elKwp, nf(s.kwp, 2));
      set(elArea, nf(s.n * 2));
      set(elInv, nf(inversorPara(s.kwp)));
      set(elProd, nf(s.prod) + ' kWh');
    ...
    function pintarSaidas(s) {
      ...
      set(document.getElementById('e-amort'), anos ? nf(anos, 1) : '--');
      set(document.getElementById('e-poup'), nf(s.poup1) + ' €');
      set(document.getElementById('e-tir'), nf(tir(g.preco, s.poup1, 10) * 100, 1) + '%');
      set(document.getElementById('e-25'), nf(Math.round(acumulado - g.preco)) + ' €');
    }
    ...
    var PASSOS = [
      'A ler a ficha do cliente...',
      'A importar a produção do PVSOL...',
      'A consultar a tabela de preços...',
      'A correr o modelo económico a 25 anos...',
      'A montar os dois documentos...'
    ];

    btn.addEventListener('click', function () {
      btn.disabled = true;
      var i = 0;
      btn.textContent = PASSOS[0];
      var itv = setInterval(function () {
        i++;
        if (i < PASSOS.length) btn.textContent = PASSOS[i];
      }, 600);
      setTimeout(function () {
        clearInterval(itv);
        gerado = true;
  ...
  global.backoffice = function () {
    var tabs = document.querySelectorAll('.bo-tab');
    if (!tabs.length) return;
    Array.prototype.forEach.call(tabs, function (t) {
      t.addEventListener('click', function () {
        var alvo = t.getAttribute('data-bo');
        Array.prototype.forEach.call(tabs, function (x) { x.classList.toggle('on', x === t); });
        Array.prototype.forEach.call(document.querySelectorAll('.bo-panel'), function (p) {
          p.classList.toggle('on', p.id === 'bo-' + alvo);
        });
      });
    });
  };
